...
| `auto-rename.ts`                              | **Logique pure de l'auto-rename (B6)** : `rolesAjoutes` (diff d'ensembles), `styleDeclenche` (quel style appliquer suite à un changement de rôles, priorité = ordre du mapping), `styleEffectif` (priorité **opt-out > signature membre > style du rôle**, [ADR-0007](../../docs/decisions/0007-opt-out-membre.md)), `parseStyleName`, type `MappingRoleStyle`. Aucun import discord.js. Voir [ADR-0004](../../docs/decisions/0004-auto-rename.md). |
