...
describe('commande /auto-rename', () => {
  it('se nomme "auto-rename", a une description et exige ManageGuild', () => {
    const cmd = creerAutoRenameCommand(fakeStore());
  ...
  it('expose les 3 sous-commandes add / remove / list', () => {
    const json = creerAutoRenameCommand(fakeStore()).data.toJSON();
    const noms = (json.options ?? []).map((o) => o.name).sort();
    expect(noms).toEqual(['add', 'list', 'remove']);
  });
